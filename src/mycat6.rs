...
fn io_blocksize(file_fd: i32) -> usize {
    ...
    // 根据实验结果，使用128倍页面大小作为最优缓冲区大小
    let base_size = lcm(page_size, fs_block_size);
    let optimal_multiplier = 128;
    
    // 确保缓冲区大小至少是base_size的optimal_multiplier倍
    let optimal_size = base_size * optimal_multiplier;
    
    // 但也不要过大，限制在1MB以内
    std::cmp::min(optimal_size, 1024 * 1024)
}
...
fn main() {
    ...
    // 使用fadvise提示内核我们将顺序读取整个文件
    unsafe {
        // POSIX_FADV_SEQUENTIAL: 顺序访问模式，内核会增加readahead窗口
        // POSIX_FADV_WILLNEED: 告诉内核我们很快会需要这些数据
